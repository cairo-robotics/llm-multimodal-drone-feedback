...
function updateGameArea() {
  ...
  /*  if (myGameArea.x && myGameArea.y) {
    ...
    if (myGameDrone.stopGame(landingZone)) {
        ...
        // log += [myGameDrone.x, myGameDrone.y, XSpeed, YSpeed, ux, uy, myGameDrone.angle, ang_vel, "\n"];
...
function dronePosition() {
    ...
    return [Number(myGameDronex).toFixed(3), Number(myGameDroney).toFixed(3)];
}
function droneAngle() {
    myGameDroneAngle = -1* theta_angle * (180/ Math.PI);
    return Number(myGameDroneAngle).toFixed(3);
}
